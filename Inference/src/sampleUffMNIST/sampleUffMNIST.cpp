...
bool SampleUffMNIST::infer()
{
	...
	float total = 0;
	float tru_count = 0;
	std::string a = locateFile("labels.txt", mParams.dataDirs);
	
	std::ifstream t(a);
	std::stringstream buffer_label;
	buffer_label << t.rdbuf();
	std::string s = buffer_label.str();
	auto string_to_print = explode(s, ' ');
	

	// Try to infer each digit 0-9
	for (int digit = 0; digit < string_to_print.size(); digit++)
	{
		//std::cout << stoi(string_to_print[digit]) << '\n';
		...
		outputCorrect = verifyOutput(buffers, mParams.outputTensorNames[0], stoi(string_to_print[digit]));
	...
	tru_count =(100*tru_count) / string_to_print.size();
	total = total / 1000;
	...
	cout << '\n';
	cout << "++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++"<<"\n";
	cout << "+++++++++++++++++++ Accuracy of the Test Dataset is " << tru_count << "% +++++++++++++++++"<<"\n";
	cout << "++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++";
	cout << '\n';
	cout << '\n';
	cout << '\n';
	cout << '\n';
	cout << '\n';
	gLogInfo << "Total to run a single batch time over " << string_to_print.size() << " test_dataset images is " << total << "s" << std::endl;
...
samplesCommon::UffSampleParams initializeSampleParams(const samplesCommon::Args & args)
{
	...
	{
		params.dataDirs.push_back("C:/Users/PRATHAMESH/Desktop/11/Inference/data/mnist/");
	...
	params.uffFileName = locateFile("uff_model.uff", params.dataDirs);
	params.inputTensorNames.push_back("Placeholder");
	params.batchSize = 1;
